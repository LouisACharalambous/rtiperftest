...
fastDDS_cmake_location="${script_location}/resource/fastDDS/CMakeLists.txt"
cycloneDDS_cmake_location="${script_location}/resource/cycloneDDS/CMakeLists.txt"
...
# We don't build FastDDS Either
BUILD_FASTDDS=0

# We don't build CycloneDDS Either
BUILD_CYCLONEDDS=0
...
function clean()
{
    ...
    rm -rf "${script_location}"/srcCpp/fastDDS/perftest.*
...
function executable_checking()
{
    ...
    if [ "${BUILD_FASTDDS}" -eq "1" ]; then

        # Is fastrtpsgen in the path?
        if [ -z `which fastrtpsgen` ]
        then
            echo -e "${ERROR_TAG} fastrtpsgen executable not found."
            exit -1
        fi
    ...
    if [ "${BUILD_CYCLONEDDS}" -eq "1" ]; then

        # CYCLONEDDSHOME variable is required.
        if [ -z "${CYCLONEDDSHOME}" ]; then
            echo -e "${ERROR_TAG} CYCLONEDDSHOME not set or path does not exist"
            exit -1
        fi

        if [ -z `which "${JAVA_EXE}"` ]; then
            echo -e "${ERROR_TAG} java executable not found."
            exit -1
        fi
...
function build_fastDDS_cpp()
{
    # Delete files generated by cmake previously.
    rm -rf fastDDS_build

    # Generation command
    gen_command="\"${gen_executable}\" -d \"${classic_cpp_folder}/fastDDS\" \"${idl_location}/perftest.idl\" "

    echo ""
    echo -e "${INFO_TAG} Generating types for C++ For FastDDS."
    echo -e "${INFO_TAG} Command: $gen_command"

    # Executing GEN command here.
    eval $gen_command
    if [ "$?" != 0 ]; then
        echo -e "${ERROR_TAG} Failure generating code for FastDDS."
        exit -1
    fi
    ...
    eval $cmake_generate_command
    if [ "$?" != 0 ]; then
        echo -e "${ERROR_TAG} Failure generating makefiles with cmake for FastDDS."
        rm -rf ${script_location}/CMakeLists.txt
        exit -1
    fi

    cmake_build_command="${CMAKE_EXE} --build ./fastDDS_build --config ${RELEASE_DEBUG} --target FastDDSperftest_cpp"
...
function build_cycloneDDS_cpp()
{
    ...
    eval $gen_command
    if [ "$?" != 0 ]; then
        echo -e "${ERROR_TAG} Failure generating code for CycloneDDS."
        exit -1
    fi

    echo ""
    echo -e "${INFO_TAG} Copy CMakeListst.txt file from: $cycloneDDS_cmake_location"
    cp $cycloneDDS_cmake_location ${script_location}/

    cmake_generate_command="${CMAKE_EXE} -DCMAKE_BUILD_TYPE=${RELEASE_DEBUG} -G \"Unix Makefiles\" -B./cycloneDDS_build -H."
    ...
    cmake_build_command="${CMAKE_EXE} --build ./cycloneDDS_build --config ${RELEASE_DEBUG} --target CycloneDDSperftest_cpp"
...
if [ "${BUILD_FASTDDS}" -eq "1" ]; then
    ...
    gen_executable="fastrtpsgen"

    classic_cpp_lang_string=C++
    build_fastDDS_cpp

# Build for ConnextDDS Micro
elif [ "${BUILD_CYCLONEDDS}" -eq "1" ]; then

    classic_cpp_lang_string=C++
    build_cycloneDDS_cpp
